...
trait RestApi extends HttpService with ActorLogging { actor: Actor =>
  ...
  def login(uid: String): String = {
    var nonceString = generateSecureRandomNumber()
    userNonceMap += (uid -> nonceString)
    nonceString
  }

  def generateSecureRandomNumber() :String ={
  ...
  def issuetoken(userid: String): String = {
    ...
    var str=generateSecureRandomNumber()+userid
    val claimsSet = JwtClaimsSet(Map("exp" -> "3600", "userid" -> str))
